...
def spider_html_info(url):
    try:
        ...
        # print ("source = ", source)
        if(title and publish_time and source and article_text):
            sonews = {
            'title': title,
            'DATE': publish_time,
            'source': source,
            ...
            result = collection.insert(sonews)
            print(result)
        #writer.writerow((title, publish_time, url, article_text, source))
    except:
        pass

    if url == 'http://www.chinanpo.gov.cn/1944/124000/index.html':
        #fp.close()
        # 获取结束时的时间
        endTime = time.time()
...
# ----------------------------------------------主函数----------------------------------------------
def main():
    url = "http://www.chinanpo.gov.cn/1944/126458/nextindex.html"  # 125177第一篇文章
    count = 1
    while True:
